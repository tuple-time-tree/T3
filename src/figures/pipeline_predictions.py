...
def benchmark_size_reports() -> list[tuple[str, dict]]:
    ...
    for model, name in (
        (optimize_per_tuple_tree_model(benchmarks), "T3 (Pipeline Vectors)"),
        (optimize_flat_tree_model(benchmarks), "Flat Vector"),
    ):
...
def eval_benchmarks(results):
    ...
    plt.savefig(f"{get_figure_path()}/per_pipeline_model.{get_figure_format()}", bbox_inches="tight")


def pipeline_prediction_figure():
