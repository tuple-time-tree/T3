...
void Model::predictManyCompiled() {
    if (currentlyFilled > 100) {
        uint64_t numThreads = 32;
        uint64_t chunkSize = currentlyFilled / numThreads;
        uint64_t remainder = currentlyFilled % numThreads;
        std::vector<std::thread> threadPool;
        uint64_t start = 0;
        for (int i = 0; i < numThreads; ++i) {
            uint64_t end = start + chunkSize + (i < remainder ? 1 : 0);
            threadPool.emplace_back(processChunk, std::ref(data), std::ref(out), start, end, nFeatures);
            start = end;
        }
